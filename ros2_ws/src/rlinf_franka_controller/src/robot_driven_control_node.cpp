...
// This value is used when configuring the main loop to use SCHED_FIFO
// scheduling We use a midpoint RT priority to allow maximum flexibility to
// users
...
int main(int argc, char **argv) {
  ...
  const bool robot_driven_loop =
      cm->get_parameter_or("robot_driven_loop", true);

  const bool has_realtime = realtime_tools::has_realtime_kernel();
  const bool lock_memory =
      cm->get_parameter_or<bool>("lock_memory", has_realtime);
  if (lock_memory) {
    const auto lock_result = realtime_tools::lock_memory();
    if (!lock_result.first) {
      RCLCPP_WARN(cm->get_logger(), "Unable to lock the memory: '%s'",
  ...
  const int thread_priority =
      cm->get_parameter_or<int>("thread_priority", kSchedPriority);
  RCLCPP_INFO(cm->get_logger(),
              "Spawning %s RT thread with scheduler priority: %d",
              cm->get_name(), thread_priority);
  RCLCPP_INFO(cm->get_logger(), "Control loop timing: %s",
              robot_driven_loop ? "robot-driven (fixed-rate sleep disabled)"
                                : "fixed-rate");

  std::thread cm_thread([cm, robot_driven_loop, thread_priority,
                         use_sim_time]() {
    rclcpp::Parameter cpu_affinity_param;
    if (cm->get_parameter("cpu_affinity", cpu_affinity_param)) {
      std::vector<int> cpus = {};
      if (cpu_affinity_param.get_type() ==
          rclcpp::ParameterType::PARAMETER_INTEGER) {
        cpus = {static_cast<int>(cpu_affinity_param.as_int())};
      } else if (cpu_affinity_param.get_type() ==
                 rclcpp::ParameterType::PARAMETER_INTEGER_ARRAY) {
        const auto cpu_affinity_param_array =
            cpu_affinity_param.as_integer_array();
        std::for_each(
      ...
      const auto affinity_result =
          realtime_tools::set_current_thread_affinity(cpus);
      if (!affinity_result.first) {
        RCLCPP_WARN(cm->get_logger(), "Unable to set the CPU affinity : '%s'",
    ...
    while (rclcpp::ok()) {
      ...
      // Franka's blocking read supplies the 1 kHz cycle boundary.
      if (!robot_driven_loop) {
