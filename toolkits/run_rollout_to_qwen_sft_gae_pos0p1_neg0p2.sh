...
DATASET_DIR="${REPO_PATH}/logs/qwen3_vl_4b_5frame_dualview_rollout_gae_pos0p1_neg0p2_balanced"
CONFIG_NAME="qwen3_vl_4b_5frame_dualview_rollout_gae_pos0p1_neg0p2_sft_10000"
PIPELINE_LOG="${REPO_PATH}/logs/rollout_to_qwen_sft_gae_pos0p1_neg0p2_$(date +'%Y%m%d-%H%M%S').log"
TARGET_STEP=299
...
log "score_source=gae_target positive_threshold=0.1 negative_threshold=0.2"

count="$(count_step_files)"
if [[ "${count}" -lt 8 ]]; then
  log "target step files are incomplete: count=${count}/8"
  exit 1
fi
