...
GELLO 夹爪值是连续量：``0`` 对应 Robotiq 完全张开，``1`` 对应完全闭合，
中间值会保留部分开合状态（例如 ``0.5`` 对应半开位置）。
如果夹爪在机械限位处需要更大的力，可在 ``override_cfg`` 中分别设置
``left_gripper_close_force`` 或 ``right_gripper_close_force``，取值范围为
``[0, 255]``。该力仅用于完全闭合目标，中间位置仍为连续控制。应使用能够
可靠闭合夹爪的最小力。
