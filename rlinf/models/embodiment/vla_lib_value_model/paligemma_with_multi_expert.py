"""PaliGemma with single or multiple experts sharing the same VLM backbone.

Contains two model classes:
- PaliGemmaWithExpertModel: Single action expert (from openpi)
- PaliGemmaWithMultiExpertModel: Multiple named experts (from openpi05)

Both support interleaved attention between VLM backbone and expert transformers,
with optional knowledge insulation (stop gradient from expert to VLM).
...
class PaliGemmaWithMultiExpertModel(nn.Module):
    ...
    def __init__(
        ...
        self.trainable_experts = trainable_experts if trainable_experts is not None else self.expert_names
        ...
        logger.info(f"Freeze vision encoder: {freeze_vision_encoder}, Freeze VLM: {freeze_vlm}")

        if use_adarms is None:
            use_adarms = [False, {name: False for name in self.expert_names}]
    ...
    def forward(
        ...
        return self._forward_interleaved(
            inputs_embeds, attention_mask, position_ids, adarms_cond, detach_prefix_for_suffix, expert
        )
    ...
    def _resolve_expert_name(self, expert_name: str | None) -> str:
        ...
        if expert_name is not None:
            if expert_name not in self.expert_names:
                raise ValueError(f"Unknown expert: {expert_name}. Available: {self.expert_names}")
            return expert_name
    ...
    def load_state_dict(self, state_dict, strict=True, assign=False):
        ...
        result = super().load_state_dict(remapped_state_dict, strict=strict, assign=assign)
        ...
        if missing_keys:
            ...
            missing_prefixes = set(k.split('.')[0] for k in missing_keys)
            logger.info(f"  Missing (will be randomly initialized): {len(missing_keys)} params from {missing_prefixes}")
        if unexpected_keys:
            unexpected_prefixes = set(k.split('.')[0] for k in unexpected_keys)
            logger.info(f"  Unexpected (ignored): {len(unexpected_keys)} params from {unexpected_prefixes}")
        if ignored_keys:
    ...
    def state_dict(self, *args, destination=None, prefix='', keep_vars=False, **kwargs):
        ...
        """
        sd = super().state_dict(*args, destination=destination, prefix=prefix, keep_vars=keep_vars, **kwargs)

        if os.environ.get("PALIGEMMA_COMPAT_STATE_DICT", 0) == "1" and "action" in self.expert_names:
            compat_sd = {}
            for key, value in sd.items():
                if key.startswith(f"{prefix}experts.action."):
                    new_key = key.replace(f"{prefix}experts.action.", f"{prefix}gemma_expert.", 1)
