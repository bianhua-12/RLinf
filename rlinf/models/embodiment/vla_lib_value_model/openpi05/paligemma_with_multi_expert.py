...
Supports multiple expert transformers (e.g., action expert, critic, value head) that:
1. Share the same PaliGemma vision-language backbone
2. Receive information via KV cache from PaliGemma
3. Each expert can have its own config and use_adarms setting
...
from ..openpi.transformers_replace.models.siglip.check import check_transformers_replace
...
class PaliGemmaWithMultiExpertModel(nn.Module):
    ...
    def __init__(
        self,
        vlm_config=None,
        expert_configs: dict | None = None,
        use_adarms: list | None = None,
        precision: Literal["bfloat16", "float32"] = "bfloat16",
        freeze_vision_encoder: bool = False,
        freeze_vlm: bool = False,
        trainable_experts: list[str] | None = None,
        backbone_variant: Literal["paligemma_2b", "paligemma_800m"] = "paligemma_2b",
        ...
        if backbone_variant == "paligemma_2b":
            check_transformers_replace()
            if vlm_config is None:
                raise ValueError(
                    "vlm_config must be provided when backbone_variant='paligemma_2b'"
                )
        ...
        elif backbone_variant == "paligemma_800m":
            if not siglip_path or not gemma3_path:
                raise ValueError(
                    "siglip_path and gemma3_path must be provided when backbone_variant='paligemma_800m'"
                )
            ...
            siglip_hidden = self.vision_tower.config.hidden_size
        ...
        for name, expert_config in expert_configs.items():
            ...
            if (
                backbone_variant == "paligemma_800m"
                and expert_config.head_dim != self.gemma3.config.head_dim
            ):
                raise ValueError(
                    f"Expert '{name}' has head_dim={expert_config.head_dim}, "
                    f"but Gemma3 has head_dim={self.gemma3.config.head_dim}"
                )
    ...
    def forward(
        ...
        """
        if self.backbone_variant == "paligemma_800m":
            return self._forward_two_stage_800m(
    ...
    def _forward_two_stage_800m(
        ...
        """Forward for paligemma_800m backbone (two-stage only)."""
        ...
        raise ValueError(
            "backbone_variant='paligemma_800m' does not support interleaved forward. "
            "Use two-stage calls with inputs_embeds=[prefix, None] then [None, suffix]."
        )
    ...
    def set_requires_grad(self):
        ...
        if self.freeze_vision_encoder:
            vision_tower = (
                self.paligemma.vision_tower
                if self.backbone_variant == "paligemma_2b"
                else self.vision_tower
            )
...
class PaliGemmaWithMultiExpert800M(PaliGemmaWithMultiExpertModel):
    """Backwards-compatible wrapper for 800M backbone.

    This wrapper preserves the historical constructor while using the unified
    multi-expert implementation in this module.
    """
    ...
    def __init__(
        ...
        super().__init__(
            ...
            use_adarms=[False, dict.fromkeys(expert_configs.keys(), False)],
            precision=precision,
