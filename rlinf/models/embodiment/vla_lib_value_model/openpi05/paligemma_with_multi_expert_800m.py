"""PaliGemmaWithMultiExpert800M: SigLIP2 + Gemma3 270M backbone with independent experts.
...
from transformers import GemmaForCausalLM, SiglipVisionModel
...
class PaliGemmaWithMultiExpert800M(nn.Module):
    ...
    def __init__(
        ...
        logger.info(
            f"Creating PaliGemmaWithMultiExpert800M: experts={self.expert_names}, "
    ...
    def embed_image(self, image: torch.Tensor) -> torch.Tensor:
        ...
        """
        feats = self.vision_tower(pixel_values=image).last_hidden_state  # [B, 256, 1152]
    ...
    def forward(
        ...
        **kwargs,  # accept & ignore: adarms_cond, detach_prefix_for_suffix, etc.
        ...
        """Two-stage forward for 800M backbone.

        Unlike PaliGemmaWithMultiExpertModel (2B), interleaved forward is NOT supported
        ...
        # (transformers source: line 33 `if use_cache and past_key_values is None and not self.training`).
        ...
        raise ValueError(
            "PaliGemmaWithMultiExpert800M does not support interleaved forward "
            ...
            "(always active when backbone_variant='paligemma_800m')."
        )
