...
def make_franka_example() -> dict:
    ...
    return {
        ...
        "observation/images/left_cam": np.random.randint(
            256, size=(224, 224, 3), dtype=np.uint8
        ),
        "observation/images/right_cam": np.random.randint(
            256, size=(224, 224, 3), dtype=np.uint8
        ),
...
class FrankaInputs(_transforms.DataTransformFn):
    ...
    Converts:
        ...
        - Images: Maps 3 cameras (left_cam, right_cam, wrist_cam) - all active
    """
    ...
    def __init__(self, model_type: str = "pi05"):
        ...
        Args:
            model_type: Model type ("pi0", "pi05", "pi0_fast") for consistency.
                       Note: All 3 cameras are active regardless of model type.
        """
    ...
    def __call__(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        # Parse images
        left_image = _parse_image(data["observation/images/left_cam"])
        right_image = _parse_image(data["observation/images/right_cam"])
        wrist_image = _parse_image(data["observation/images/wrist_cam"])

        # Convert to tensor CHW format
        left_image = self._process_image(left_image)
        right_image = self._process_image(right_image)
        wrist_image = self._process_image(wrist_image)

        # Set up image inputs based on model type (all 3 cameras active)
        # Map to OpenPI's default IMAGE_KEYS for compatibility with PI0ImageProcessor
        # left_cam -> base_0_rgb, right_cam -> right_wrist_0_rgb, wrist_cam -> left_wrist_0_rgb
        names = ("base_0_rgb", "left_wrist_0_rgb", "right_wrist_0_rgb")
        images = (left_image, wrist_image, right_image)
