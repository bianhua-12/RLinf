...
class TestExpertValueModes:
    ...
    def test_contains_expert_mse(self):
        assert "expert_mse" in EXPERT_VALUE_MODES

    def test_contains_expert_categorical(self):
        assert "expert_categorical" in EXPERT_VALUE_MODES

    def test_contains_expert_distributional(self):
        assert "expert_distributional" in EXPERT_VALUE_MODES

    def test_has_exactly_three_modes(self):
        assert len(EXPERT_VALUE_MODES) == 3
...
class TestValuePolicyModeFlags:
    ...
    @pytest.mark.parametrize(
        "mode", ["expert_mse", "expert_categorical", "expert_distributional"]
    )
    def test_expert_modes_set_flag_true(self, _make_policy, mode):
        policy = _make_policy(mode)
        assert policy._is_expert_mode is True

    @pytest.mark.parametrize("mode", ["continuous", "discrete"])
    def test_vlm_modes_set_flag_false(self, _make_policy, mode):
        policy = _make_policy(mode)
        assert policy._is_expert_mode is False

    def test_value_mode_stored(self, _make_policy):
        policy = _make_policy("continuous")
        assert policy.value_mode == "continuous"
...
class TestPI05CriticConfig:
    ...
    def _import_config(self):
        from rlinf.models.embodiment.vla_lib_value_model.modeling_critic import (
            PI05CriticConfig,
        )
    ...
    def test_default_critic_forward_mode(self):
        config = self.PI05CriticConfig()
        assert config.critic_forward_mode == "expert"

    def test_default_expert_loss_type(self):
        config = self.PI05CriticConfig()
        assert config.expert_loss_type == "mse"
    ...
    def test_default_vlm_loss_weight(self):
        config = self.PI05CriticConfig()
        assert config.vlm_loss_weight == 1.0

    def test_default_expert_loss_weight(self):
        config = self.PI05CriticConfig()
        assert config.expert_loss_weight == 1.0

    def test_custom_values(self):
        config = self.PI05CriticConfig(
            critic_expert_variant="gemma_300m",
            critic_forward_mode="dual",
        ...
        assert config.critic_forward_mode == "dual"
        assert config.expert_loss_type == "categorical"
        assert config.num_bins == 101
        assert config.v_min == -2.0
        assert config.v_max == 2.0
        assert config.vlm_loss_weight == 0.5
        assert config.expert_loss_weight == 2.0

    def test_inherits_from_pi05config(self):
        from rlinf.models.embodiment.vla_lib_value_model.openpi05.configuration_pi05 import PI05Config
...
class TestCriticOutput:
    ...
    def test_default_construction_all_none(self):
        ...
        assert output.language_loss is None
        assert output.language_logits is None
        assert output.language_token_acc is None
        assert output.language_loss_mask is None
...
class TestValueHead:
    ...
    def test_mse_mode_output_dim_1(self):
        head = self.ValueHead(
            hidden_size=64, num_bins=201, loss_type="mse", v_min=-1.0, v_max=0.0
        )
        assert head.value_proj.out_features == 1

    def test_categorical_mode_output_dim_num_bins(self):
        head = self.ValueHead(
            hidden_size=64,
            num_bins=201,
    ...
    def test_distributional_mode_output_dim_num_bins(self):
        head = self.ValueHead(
            hidden_size=64,
            num_bins=101,
        ...
        assert head.value_proj.out_features == 101
    ...
    def test_atoms_none_for_mse(self):
        head = self.ValueHead(
            hidden_size=64, num_bins=201, loss_type="mse", v_min=-1.0, v_max=0.0
        )
        assert head.atoms is None

    def test_atoms_shape_for_categorical(self):
        head = self.ValueHead(
            hidden_size=64,
            num_bins=201,
            loss_type="categorical",
    ...
    def test_atoms_range_for_categorical(self):
        head = self.ValueHead(
            hidden_size=64,
            num_bins=201,
            loss_type="categorical",
    ...
    def test_cls_embedding_batch_size_1(self):
        head = self.ValueHead(
            hidden_size=32, num_bins=10, loss_type="mse", v_min=0.0, v_max=1.0
        )
        cls_emb = head.get_cls_embedding(batch_size=1)
        assert cls_emb.shape == (1, 1, 32)

    def test_delta_z_computed_for_categorical(self):
        head = self.ValueHead(
            hidden_size=64,
            num_bins=201,
            loss_type="categorical",
